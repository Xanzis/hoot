...
impl Object {
    ...
    pub fn as_list(self) -> Option<(ObjectReference, ObjectReference)> {
        match self {
            Object::Cons(a, b) => Some((a, b)),
            _ => None,
        }
    }
}
...
impl Environment {
    ...
    pub fn push_symbol(&mut self, name: String) -> Value {
        ...
        if let Some(k) = builtin::Keyword::get(&name) {
            ...
            let sref = self
                .symbol_names
                .entry(name)
                .or_insert(SymbolReference(next_num));
            Value::Symbol(*sref)
        }
    }
    ...
    pub fn list_as_2(
        ...
        // attempt to pull 2 values off the head of the given list
        if let Object::Cons(a, next) = self.get_object(list) {
            if let Object::Cons(b, _) = self.get_object(next) {
                return Some((a, b));
            }
        }
